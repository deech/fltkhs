...
{# fun Fl_Hor_Slider_New_WithLabel as horSliderNewWithLabel' { `Int',`Int',`Int',`Int', `CString'} -> `Ptr ()' id #}
...
horSliderNew rectangle l' =
    ...
    in case l' of
        Nothing -> horSliderNew' x_pos y_pos width height >>=
                             toRef
        Just l -> do
          ref <- copyTextToCString l >>= \l' -> horSliderNewWithLabel' x_pos y_pos width height l' >>= toRef
