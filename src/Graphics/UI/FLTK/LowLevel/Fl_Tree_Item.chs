...
instance (Parent a TreeItem, impl ~ (Ref a -> Ref a -> IO (Int))) => Op (SwapChildrenByTreeItem ()) TreeItem orig impl where
  runOp _ _ tree_item a b = withRef tree_item $ \tree_itemPtr -> withRef a $ \aPtr -> withRef b $ \bPtr -> swapChildrenByTreeItem' tree_itemPtr aPtr bPtr
...
{# fun unsafe Fl_Tree_Item_visible as visible' { id `Ptr ()' } -> `Int' #}
instance (impl ~ (IO (Int))) => Op (Visible ()) TreeItem orig impl where
  runOp _ _ tree_item = withRef tree_item $ \tree_itemPtr -> visible' tree_itemPtr
{# fun unsafe Fl_Tree_Item_is_visible as isVisible' { id `Ptr ()' } -> `Int' #}
instance (impl ~ (IO (Int))) => Op (IsVisible ()) TreeItem orig impl where
  runOp _ _ tree_item = withRef tree_item $ \tree_itemPtr -> isVisible' tree_itemPtr
{# fun unsafe Fl_Tree_Item_visible_r as visibleR' { id `Ptr ()' } -> `Int' #}
instance (impl ~ (IO (Int))) => Op (VisibleR ()) TreeItem orig impl where
...
{# fun unsafe Fl_Tree_Item_is_root as isRoot' { id `Ptr ()' } -> `Int' #}
instance (impl ~ (IO (Int))) => Op (IsRoot ()) TreeItem orig impl where
