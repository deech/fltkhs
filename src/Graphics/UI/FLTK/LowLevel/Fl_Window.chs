...
{# fun unsafe Fl_Window_handle as windowHandle' {id `Ptr ()', cFromEnum `Event'} -> `Bool' cToBool #}
windowHandle :: WindowPtr -> Event -> IO Bool
windowHandle windowPtr event = withForeignPtr windowPtr
                               (\p -> (windowHandle' (castPtr p) event))
{# fun unsafe Fl_Window_New_WithLabel as windowNewWithLabel' {`Int', `Int', `String'} -> `Ptr ()' id #}
