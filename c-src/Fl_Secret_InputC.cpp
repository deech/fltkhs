...
void Fl_DerivedSecret_Input::destroy_data(){
  if (this->destroy != NULL){
    ...
    this->destroy((fl_Secret_Input)this,res);
    if (fps) { free(fps); }
    free(res);
  }
}
EXPORT {
  ...
  FL_EXPORT_C(void*,Fl_Secret_Input_user_data)(fl_Secret_Input secret_input){
    C_to_Fl_Callback* stored_cb = (static_cast<C_to_Fl_Callback*>((static_cast<Fl_DerivedSecret_Input*>(secret_input))->user_data()));
    if(stored_cb){
      return stored_cb->get_user_data();
    }
    else {
      return (static_cast<Fl_DerivedSecret_Input*>(secret_input))->user_data();
    }
  }
