...
EXPORT {
  ...
  FL_EXPORT_C(void*,Fl_Round_Clock_user_data)(fl_Round_Clock b){
    C_to_Fl_Callback* stored_cb = (static_cast<C_to_Fl_Callback*>((static_cast<Fl_DerivedRound_Clock*>(b))->user_data()));
    if(stored_cb){
      return stored_cb->get_user_data();
    }
    else {
      return (static_cast<Fl_DerivedRound_Clock*>(b))->user_data();
    }
  }
  ...
  FL_EXPORT_C(void,Fl_Round_Clock_draw_label_with_xywh_alignment)(fl_Round_Clock b,int x,int y,int w,int h,Fl_Align alignment){
    (static_cast<Fl_DerivedRound_Clock*>(b))->draw_label(x,y,w,h,alignment);
  }
  FL_EXPORT_C(void,Fl_Round_Clock_measure_label)(fl_Round_Clock b,int* ww,int* hh){
    (static_cast<Fl_DerivedRound_Clock*>(b))->measure_label(*ww,*hh);
  }

  FL_EXPORT_C(fl_Window,    Fl_Round_Clock_window)(fl_Round_Clock round_clock){
    return (fl_Window) (static_cast<Fl_DerivedRound_Clock*>(round_clock))->window();
  }
  FL_EXPORT_C(fl_Window,    Fl_Round_Clock_top_window)(fl_Round_Clock round_clock){
    return (fl_Window) (static_cast<Fl_DerivedRound_Clock*>(round_clock))->top_window();
  }
  FL_EXPORT_C(fl_Window ,   Fl_Round_Clock_top_window_offset)(fl_Round_Clock round_clock, int* xoff, int* yoff){
    return (fl_Window) (static_cast<Fl_DerivedRound_Clock*>(round_clock))->top_window_offset(*xoff,*yoff);
  }
  FL_EXPORT_C(fl_Group,Fl_Round_Clock_as_group)(fl_Round_Clock round_clock){
    return (fl_Group) (static_cast<Fl_DerivedRound_Clock*>(round_clock))->as_group();
  }
  FL_EXPORT_C(fl_Gl_Window,Fl_Round_Clock_as_gl_window)(fl_Round_Clock round_clock){
    return (fl_Gl_Window) (static_cast<Fl_DerivedRound_Clock*>(round_clock))->as_gl_window();
  }
  FL_EXPORT_C(fl_Round_Clock, Fl_Round_Clock_New_WithLabel)(int x, int y, int w, int h, const char* label, Destroy_Function_Pointers dfps) {
    Fl_DerivedRound_Clock* button = new Fl_DerivedRound_Clock(x,y,w,h,label,dfps);
    return (static_cast<fl_Round_Clock>(button));
  }
  FL_EXPORT_C(fl_Round_Clock, Fl_Round_Clock_New)(int x, int y, int w, int h, Destroy_Function_Pointers dfps) {
    Fl_DerivedRound_Clock* button = new Fl_DerivedRound_Clock(x,y,w,h,0,dfps);
    return (fl_Round_Clock)button;
  }
  FL_EXPORT_C(void,Fl_Round_Clock_Destroy)(fl_Round_Clock button){
    delete (static_cast<Fl_DerivedRound_Clock*>(button));
  }
  FL_EXPORT_C(void,Fl_Round_Clock_set_value)(fl_Round_Clock round_clock,ulong v){
    return (static_cast<Fl_DerivedRound_Clock*>(round_clock))->value(v);
  }
  FL_EXPORT_C(void,Fl_Round_Clock_set_value_with_hms)(fl_Round_Clock round_clock,int H,int m,int s){
    return (static_cast<Fl_DerivedRound_Clock*>(round_clock))->value(H,m,s);
  }
  FL_EXPORT_C(ulong,Fl_Round_Clock_value)(fl_Round_Clock round_clock){
    return (static_cast<Fl_DerivedRound_Clock*>(round_clock))->value();
  }
  FL_EXPORT_C(int,Fl_Round_Clock_hour)(fl_Round_Clock round_clock){
    return (static_cast<Fl_DerivedRound_Clock*>(round_clock))->hour();
  }
  FL_EXPORT_C(int,Fl_Round_Clock_minute)(fl_Round_Clock round_clock){
    return (static_cast<Fl_DerivedRound_Clock*>(round_clock))->minute();
  }
  FL_EXPORT_C(int,Fl_Round_Clock_second)(fl_Round_Clock round_clock){
    return (static_cast<Fl_DerivedRound_Clock*>(round_clock))->second();
  }
#ifdef __cplusplus
}
#endif
